...
/// Output of VAD segmentation for a chunk of audio.
/// The boundaries are expressed in sample indices so callers can convert
/// to timestamps using the known sample rate.
pub struct SpeechSegment {
    pub start_sample: usize,
    pub end_sample: usize,
    pub samples: Vec<f32>,
    /// True when a long silence preceded this segment and the ASR context
    /// should be reset before decoding.
    pub should_reset_context: bool,
}
...
impl<D: VoiceActivityDetector> VadSegmenter<D> {
    ...
    pub fn push_samples(&mut self, samples: &[f32]) -> Result<Vec<SpeechSegment>> {
    ...
    fn process_frames(&mut self, is_final_flush: bool) -> Result<Vec<SpeechSegment>> {
            ...
            if is_speech {
                ...
                if self.in_speech {
                    ...
                    segments.push(SpeechSegment {
                        start_sample: self.current_segment_start,
                        end_sample,
                        samples,
                        should_reset_context: self.current_segment_requires_reset,
                    });
        ...
        if is_final_flush {
            if self.in_speech {
                ...
                segments.push(SpeechSegment {
                    start_sample: self.current_segment_start,
                    end_sample,
                    samples: mem::take(&mut self.speech_buffer),
                    should_reset_context: self.current_segment_requires_reset,
                });
                self.in_speech = false;
